...
const controller = {
  ...
  generateQRCode: async (req, res) => {
    try {
      ...
      const sql =
        "INSERT INTO sessions (session_id, expires_at, correct_number, other_number1, other_number2) VALUES(?, ?, ?, ?, ?)";
  ...
  initiateTwoStepAuth: async (req, res) => {
    ...
    try {
      const getNumbersSql =
        "SELECT correct_number, other_number1, other_number2 FROM sessions WHERE session_id = ?";
  ...
  finalizeAuthentication: async (req, res) => {
    ...
    try {
      ...
      if (rows.length > 0 && rows[0].correct_number == selectedNumber) {
        const updateSql =
          "UPDATE sessions SET `user_id`= ?, verified = 1 WHERE session_id = ?";
  ...
  getReport: async (req, res) => {
    ...
    if (req.query.date) {
      ...
      sql = "SELECT * FROM orders WHERE DATE_FORMAT(created_at, '%Y-%m') = ?";
  ...
  update: async (req, res) => {
    ...
    try {
      ...
      const setFields = Object.keys(requestBody)
        .map((key) => `${key} = ?`)
        .join(", ");
      ...
      if (result.affectedRows === 0) {
        return res
          .status(404)
          .json({ error: "Data not found or no changes applied" });
      }
  ...
  sendNotiAll: async (req, res) => {
    ...
    if (!title || !body) {
      return res
        .status(400)
        .send({ success: false, message: "Title and body are required." });
    }
    const message = {
      notification: {
        title: title,
